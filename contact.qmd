...
(function() {
  ...
  form.addEventListener('submit', async function(event) {
    ...
    // Collect form data directly from inputs
    const checkedInterests = Array.from(interestsCheckboxes)
    ...
    try {
      const response = await fetch('https://peterebenhoch.com/api/submit-contact.php', {
        method: 'POST',
        headers: {
      ...
      if (response.ok || response.status === 200) {
        ...
        setTimeout(() => {
          const kmpCheckbox = document.getElementById('interest-kmp');
          if (kmpCheckbox) kmpCheckbox.checked = true;
        }, 100);
